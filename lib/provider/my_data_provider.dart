...
class MyData {
  Future<List<StateObject>> getStateResults() async {
    ...
    Response response = await get(Uri.parse('https://covid19-server.chrismichael.now.sh/api/v1/IndiaCasesByStates')
        );
  ...
  Future<List<DistrictObject>> getDistrictsList() async {
    ...
    Response response =
        await get(Uri.parse('https://api.covid19india.org/state_district_wise.json'));
    try {
      if (response.statusCode == 200) {
  ...
  Future<WorldWide?> getWorldWideData() async {
    WorldWide? worldWide;
    Response response = await get(Uri.parse('https://corona.lmao.ninja/v2/all'));
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body);
      worldWide = WorldWide(
  ...
  Future<List<WorldNews>> getWorldNews() async {
    ...
    Response response = await get(Uri.https('wrapapi.com','use/Shadow/covid19/news/1.0.0?wrapAPIKey=aIOK7RrdSib453QkcbWkBDW02i4Fq8TM'));
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body);
